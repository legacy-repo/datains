(ns datains.task
  "Background task scheduling via Quartzite. Individual tasks are defined in `datains.task.*`.

  ## Regarding Task Initialization:

  The most appropriate way to initialize tasks in any `datains.task.*` namespace is to implement the `task-init`
...
(defmulti init!
  ...
  once when the Quartz task scheduler is initialized. Task namespaces (`datains.task.*`) should add new
...
(defn- find-and-load-task-namespaces!
  "Search Classpath for namespaces that start with `datains.tasks.`, then `require` them so initialization can happen."
  []
