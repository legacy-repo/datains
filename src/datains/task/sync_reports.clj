          ...
          (doseq [report reports]
            ...
            (try
              (let [report-id (:id report)
                    result (tservice/sync-report report-id)
...
(def ^:private sync-reports-job-key     "datains.task.sync-reports.job")
(def ^:private sync-reports-trigger-key "datains.task.sync-reports.trigger")
