  ...
  (jdbc/with-db-transaction [t-conn *db*]
    ...
    (is (= nil
           (first (db/search-apps-with-tags
                   t-conn
                   {:query-map {:id "1"} :limit 1 :offset 0}))))
    ; db/get-app-count
    ...
    (is (= {:title "test"}
           (select-keys
            (first
             (db/search-tags
              t-conn
              {:query-map {:title "test"} :limit 1 :offset 0}))
            [:title])))
   ; db/get-tag-count
    (is (= {:count 1}
           (db/get-tag-count
    ...
    (is (= 1
           (count
            (do
              ...
              (db/connect-app-tag! t-conn {:tag-title "test" :choppy-app-id "1"})
              (db/search-apps-with-tags t-conn {:query-map {:id "1"} :limit 1 :offset 0})))))))
