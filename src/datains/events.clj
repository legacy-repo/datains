(ns datains.events
  ...
  happening throughout the Datains system in a decoupled way.

  ## Regarding Events Initialization:

  The most appropriate way to initialize event listeners in any `datains.events.*` namespace is to implement the
...
(defn- find-and-load-event-handlers!
  "Search Classpath for namespaces that start with `datains.events.`, and call their `events-init` function if it
  exists."
  []
  (log/info "is-test?: %s" config/is-test?)
  (when-not config/is-test?
    (doseq [ns-symb @u/datains-namespace-symbols
            :when   (.startsWith (name ns-symb) "datains.events.")]
